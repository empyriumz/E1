...
def load_original_model(
    ...
    Args:
        model_name: HuggingFace model identifier (e.g., "Profluent-Bio/E1-600m" or "Synthyra/Profluent-E1-600M")
    ...
    # Load model with trust_remote_code=True for custom model classes
    try:
    ...
    # Create batch preparer (the model should have one, but we create our own for consistency)
...
def evaluate_sequence_batch(
    ...
    for seq_idx, (seq_id, sequence) in enumerate(progress_bar):
        try:
            ...
            if msa_dir:
                ...
                if msa_path.exists():
                    try:
                        ...
                        logger.debug(f"Error sampling context for sequence {seq_idx}: {e}")
                        context_str = None
                        has_context = False
            ...
            original_batch = batch_preparer.get_batch_kwargs([original_input_str], device=device)
            masked_batch = batch_preparer.get_batch_kwargs([masked_input_str], device=device)
            ...
            boundary_mask = batch_preparer.get_boundary_token_mask(masked_batch["input_ids"])[0]
            ...
            masked_positions = (masked_query_tokens == mask_token_id) & (original_query_tokens != mask_token_id)
...
def evaluate(config: Dict, output_path: Optional[str] = None):
    ...
    logger.info("=" * 80)
    logger.info("Evaluating Original E1 Model (HuggingFace Compatible)")
    logger.info("=" * 80)
    ...
    if input_file is None:
        raise ValueError("Config must specify 'data.input_file' field with path to FASTA file")
    ...
    for record in SeqIO.parse(input_file, "fasta"):
        ...
        seq_str = seq_str.replace("O", "X").replace("B", "X").replace("U", "X").replace("Z", "X").replace("J", "X")
    ...
    # Load original model
    model, batch_preparer = load_original_model(model_name, device, model_dtype=model_dtype)
    ...
    logger.info(f"MSA directory: {msa_dir if msa_dir else 'None (single sequence mode)'}")
    ...
    with open(results_file, "w") as f:
        f.write("Original E1 Model Evaluation Results (HuggingFace Compatible)\n")
        f.write("=" * 80 + "\n")
        for key, value in sorted(final_results.items()):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate original E1 model (HuggingFace compatible, without fine-tuning)"
    )
    parser.add_argument(
        "--config",
        type=str,
